...
export const useOverviewCards = () => {
  ...
  const [meetingCostThisYear, setMeetingCostThisYear] = useState([
    18, 16, 5, 8, 3, 14, 14, 16, 17, 19, 18, 20,
  ]);
  const [meetingCostLastYear, setMeetingCostLastYear] = useState([
    12, 11, 4, 6, 2, 9, 9, 10, 11, 12, 13, 13,
  ]);
  const [roleOfParticipants, setRoleOfParticipants] = useState([63, 15, 22]);
  const [clevelCost, setClevelCost] = useState(
    mock({
      "data|6": [
        {
          id: "@string(8)",
